...
import type { BlogState, DateFormat, Post } from "./types";
...
export function Index({ state, posts, lanyard }: IndexProps) {
  const postIndex = [];
        ...
        >
          <div class="max-w-screen-sm h-full px-6 mx-auto flex flex-col items-center justify-center">
            ...
            <h1
              ...
              style={{
                color: state.coverTextColor,
              }}
            >
              <span
                style={{
                  color: statusMap[lanyard?.data?.discord_status],
                  marginRight: ".3rem",
                  fontSize: ".9em",
                  userSelect: "none",
                }}
                title={lanyard?.data?.discord_status || "offline"}
                aria-hidden="true"
              >
                ●
              </span>
              ...
              >
                ...
                >
                  {lanyard?.data?.listening_to_spotify ? (
                    <span>
                      <br></br>🎧 <b>Listening to:</b>{" "}
                      {lanyard?.data?.spotify?.artist.replaceAll("; ", ", ")} -{" "}
                      <a
                        target="_blank"
                    ...
                    <span>
                      <br></br>🎧 <b>Listening to:</b> nothing rn
                    </span>
                  )}
                  <br></br>
                  {lanyard?.data?.activities?.find((e) => e.type === 0) ? (
                    <span>
                      🎮 <b>Playing:</b>{" "}
                      {
                        lanyard?.data?.activities?.find((e) => e.type === 0)
